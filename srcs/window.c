...
{
	fdf->intro.__img = mlx_xpm_file_to_image(fdf->mlx.__mlx, "assets/intro.xpm", &fdf->intro.height, &fdf->intro.width);
	if (!fdf->intro.__img)
		terror(ERR_MLX_IMG);
	fdf->iso.__img = mlx_xpm_file_to_image(fdf->mlx.__mlx, "assets/iso.xpm", &fdf->iso.height, &fdf->iso.width);
	if (!fdf->iso.__img)
		terror(ERR_MLX_IMG);
	fdf->ortho.__img = mlx_xpm_file_to_image(fdf->mlx.__mlx, "assets/ortho.xpm", &fdf->ortho.height, &fdf->ortho.width);
	if (!fdf->ortho.__img)
		terror(ERR_MLX_IMG);
}
...
static void init_mlx(t_fdf *fdf)
{
	...
	if (!fdf->image.__img)
		terror(ERR_MLX_IMG);
	fdf->image.__addr = mlx_get_data_addr(fdf->image.__img,
